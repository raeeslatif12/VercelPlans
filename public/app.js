...
const showEarningNotification = notification => {
  ...
  if (notification.id) knownNotificationIds.add(Number(notification.id));
...
const loadNotifications = async ({ showNew = false } = {}) => {
  try {
    ...
    if (showNew) currentNotifications.filter(item => item.status === 'unread' && !knownNotificationIds.has(Number(item.id))).forEach(showEarningNotification);
    currentNotifications.forEach(item => knownNotificationIds.add(Number(item.id)));
...
const bottom = path => `<nav class="dashboard-nav" aria-label="Primary navigation">${[['/dashboard','⌂','Dashboard'],['/plans','▤','Plans'],['/orders','▣','My Orders'],['/tasks','★','Daily Task'],['/refer','↗','Refer & Earn'],['/withdraw','₨','Withdraw']].map(([href,icon,label]) => `<a class="${path === href ? 'active' : ''}" href="${href}"><span>${icon}</span><span>${label}</span></a>`).join('')}</nav>`;
...
const withdraw = (user, config = {}) => { const minimum = Number(config.minimumWithdrawalAmount ?? user.minimumWithdrawalAmount ?? 0); return shell(`<div class="page-heading"><span class="eyebrow">Payout center</span><h1>Withdraw</h1><p>Move available earnings to your preferred local wallet.</p></div><section class="balance-card withdraw-balance"><div><div class="balance-label">Available balance</div><div class="balance">${money(user.balance)}</div></div><a href="/history">View history →</a></section><div class="notice withdraw-requirements"><strong>Before you request a payout</strong><span>Minimum withdrawal is ${money(minimum)} and an approved active plan is required.</span></div><form class="card form-card" data-form="withdraw"><div class="field"><label>Select wallet</label><div class="choice-row">${['Easypaisa','JazzCash','SadaPay','NayaPay'].map((wallet,index) => `<button type="button" class="wallet ${index === 0 ? 'selected' : ''}" data-wallet="${wallet}">${wallet}</button>`).join('')}</div></div>${input('Account number','accountNumber','03XX XXXXXXX')}${input('Account holder name','accountHolder','As registered on the wallet')}${input('Withdrawal amount (PKR)','amount',`Minimum ${minimum}`,'number')}<button class="primary" type="submit" ${user.balance < minimum ? 'disabled' : ''}>${user.balance < minimum ? `Minimum balance ${money(minimum)}` : 'Request withdrawal'}</button></form>`, '/withdraw'); };
...
const historyPage = rows => shell(`<div class="page-heading"><span class="eyebrow">Payout activity</span><h1>Withdrawal history</h1><p>Review the date, amount and status of every payout request.</p></div><div class="card table-card">${rows.length ? `<table class="table"><thead><tr><th>Date</th><th>Amount</th><th>Status</th></tr></thead><tbody>${rows.map(row => `<tr><td data-label="Date">${new Date(row.created_at).toLocaleDateString()}</td><td data-label="Amount">${money(row.amount)}</td><td data-label="Status"><span class="status-pill ${String(row.status).toLowerCase()}">${esc(row.status)}</span></td></tr>`).join('')}</tbody></table>` : '<div class="empty-state"><strong>No withdrawals yet</strong><span>Your payout history will appear here.</span><a class="primary" href="/withdraw">Make a withdrawal</a></div>'}</div>`, '/history');
...
const fileAsDataUrl = file => new Promise((resolve, reject) => { if (!file) return reject(new Error('Please select a payment screenshot.')); const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error('Payment screenshot could not be read.')); reader.readAsDataURL(file); });
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.dataset.form === 'profile-email') {
      ...
      data.paymentProof = await fileAsDataUrl(form.elements.paymentProof.files[0]);
