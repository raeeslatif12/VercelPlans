...
const historyPage = rows => shell(`<div class="page-heading"><span class="eyebrow">Payout activity</span><h1>Withdrawal history</h1><p>Review the date, amount and status of every payout request.</p></div><div class="card table-card">${rows.length ? `<table class="table"><thead><tr><th>Date</th><th>Amount</th><th>Status</th></tr></thead><tbody>${rows.map(row => {
  ...
  const approvedAt = status === 'Approved' ? formatWithdrawalApprovalDate(row.approved_at) : null;
  const approvalLabel = status === 'Approved'
    ? (approvedAt ? `Approved on: ${approvedAt}` : 'Approval time unavailable')
    : '';
  return `<tr><td data-label="Date">${new Date(row.created_at).toLocaleDateString()}</td><td data-label="Amount">${money(row.amount)}</td><td data-label="Status"><div class="withdrawal-status-cell"><span class="status-pill ${statusClass}">${esc(status)}</span>${approvalLabel ? `<small class="withdrawal-approval-time">${esc(approvalLabel)}</small>` : ''}</div></td></tr>`;
}).join('')}</tbody></table>` : '<div class="empty-state"><strong>No withdrawals yet</strong><span>Your payout history will appear here.</span><a class="primary" href="/withdraw">Make a withdrawal</a></div>'}</div>`, '/history');
