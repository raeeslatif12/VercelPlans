...
}).join('') : '<tr class="table-empty-row"><td colspan="6"><div class="table-empty-content"><strong>No withdrawals yet</strong><span>Your payout history will appear here.</span><a class="primary" href="/withdraw">Make a withdrawal</a></div></td></tr>'}</tbody></table></div></div>`, '/history');
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.dataset.form === 'profile-email') {
      ...
      const selectedPaymentMethod = form.querySelector('[data-payment-id].selected');
      if (!selectedPaymentMethod) throw new Error('Please select a payment method.');
      const data = Object.fromEntries(new FormData(form));
      data.paymentMethodId = selectedPaymentMethod.dataset.paymentId;
