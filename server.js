...
app.get('/api/admin/orders', adminAuth, async (req, res) => res.json({ orders: (await query('SELECT o.*,u.id user_id,u.phone,u.created_at user_created_at,u.balance user_balance,p.name plan_name,pm.name payment_method FROM orders o JOIN users u ON u.id=o.user_id JOIN plans p ON p.id=o.plan_id LEFT JOIN payment_methods pm ON pm.id=o.payment_method_id ORDER BY o.created_at DESC')).rows }));
